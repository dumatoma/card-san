...
### 63. バグ修正（再修正）：店舗情報「アプリダウンロードリンクをコピー」のリンク先が誤り

**概要：** ユーザーAPP（jpcard）＞店舗情報＞「アプリダウンロードリンクをコピー」でコピーされるURLが誤っていた。#49 で一度対応したがバックエンド返却値（`member_app_download_url`）が正しい値を返さず、未設定時は App Store / Google Play のストアURLにフォールバックしていたため、指定の正規QRリンクにならなかった。クライアント指摘（2026.8.3-1）により正規リンクへ固定。

| 対象 | 修正内容 |
|------|----------|
| `jpcard/pagesA/storeInformation/storeInformation.vue` | `copy()` を `https://me-qr.com/tOELaM49` 固定に変更（バックエンド値・ストアURLフォールバックを廃止） |
| `jpcard/manifest.json` | versionName `1.1.4`→`1.1.5` / versionCode `1069`→`1070` |
...
**追記（2026-08-06 UI細調整）：** クライアント指定モックアップに合わせ、詳細画面を「Google でのクチコミの概要」（星分布バー左・平均点/星/件数右）、絞り込み（星別＋件数チップ）、並べ替え（関連度順/新規順/最高/最低）、相対時間表示（○年前等）、レビュアー頭文字アバターに調整。店舗情報のバッジも1行表示（青星アイコン＋評点＋星＋・N クチコミ＋>）に変更。詳細画面のヘッダーには店舗名を表示。

**日付：** 2026-08-03

---

### 65. 管理App TOP「クチコミ」アイコンをクライアント指定SVGに差替

**概要：** 管理App（jpshop）TOP画面の「クチコミ」アイコンが、指定デザイン（吹き出し＋星3・色 #707070）と異なっていた（2回目の指摘）。クライアント支給の SVG ファイルをそのまま採用し、デザイン・色を完全一致させた。

| 対象 | 修正内容 |
|------|----------|
| `jpshop/static/index/Review_icon_new.svg` | クライアント支給 `kuchikomi.svg`（吹き出し＋星3・fill #707070）の内容で全置換 |
...
### 66. 予約WEB（order.card-san.jp）H5 CLIビルドのUI崩れ → ロールバック

**概要：** #62 付近で予約WEB（outsiteAppointment）の指名なし表示・二重送信防止の修正を、HBuilderX非依存の uni-app CLI で H5 ビルドしデプロイしたが、生成された `index.html` に uni-app H5 の起動スクリプト（coverSupport viewport／rpx 基準設定）が欠落しており、実機でUI崩れ・予約確定ボタンが Loading のまま固まる不具合が発生（クライアント報告 2026.7.29「予約ができない」）。**本番復旧を最優先し、デプロイ直前のバックアップ（`/www/backup_order_20260727_213039.tar.gz`＝従来のHBuilderXビルド）へロールバック。** 予約は復旧（ただし指名なし・二重送信の2件は未反映に戻る）。修正ソースは維持しており、正しいビルド手順を再検討のうえ再デプロイ予定。

| 対象 | 対応 |
|------|------|
| `/www/wwwroot/order.card-san.jp`（サーバー） | 従来ビルドへロールバック（index.html＋static を復元、托管ファイル保持） |

**日付：** 2026-08-06

---

### 67. バグ修正：管理WEB 予約編集で通常メニューの「終了」時間ドラムロールが表示されない

**概要：** 管理WEB（jppc）の「予約編集」モーダルで、通常の登録メニュー（レディースカット等・`smid > 0`）の場合に「終了」時間が固定テキスト表示となり、時間選択ドラムロール（`el-time-select`）が出ず終了時間を変更できなかった。「お休み・勤務時間外」（`smid == 0`）のみドラムロールが表示され、挙動が逆転していた。

| 対象 | 修正内容 |
|------|----------|
| `jppc/jp-card-pc/src/views/appointment.vue`（予約編集ダイアログ・終了時間） | `v-if="orderDetail.smid != 0"` の固定テキスト分岐を撤廃し、常に `el-time-select` を表示（通常メニュー編集でも終了時間を変更可能に）。保存 `editKeep()` は `endTime` を送信し、終了>開始 の検証あり |
