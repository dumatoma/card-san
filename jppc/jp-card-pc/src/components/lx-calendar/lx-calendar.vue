<template>
    <div class="date">
        <div class="head">
            <div class="title">{{nowYear+'年 '+ nowMonth+'月'+nowDay+'日'}}</div>
            <div style="display: flex;align-items: center;justify-content: center;">
                <div class="icon" @click="switch_month_week('prev',true)"><text class="iconfont icon-fanhui"
                        style="color: #1A73E8;margin-right: 70upx;" /></div>
                <div class="icon" @click="switch_month_week('next',true)"><text class="iconfont next icon-fanhui"
                        style="color: #1A73E8;" /></div>
            </div>
        </div>
        <div style="box-sizing: border-box;padding-left: 150upx;padding-right: 35upx;">
            <div class="date_dl">
                <div class="dd" v-for="(item,index) in week" :key="index" :style=" item == '日' ? 'color:#D93025' : item == '土' ? 'color:#1A73E8' : '' ">{{item}}</div>
            </div>
            <swiper :style="{height:(retract ? 2 * 50 : (week_list.length + 1) * 50 ) + 'px'}" :current="current"
                circular @change="change_date">
                <swiper-item>
                    <div class="date_dl" v-show="!retract || index == to_prev_week_index"
                        v-for="(item,index) in   week_list_prev_co" :key="index">
                        <div class="dd" @click="item_click(vo,index,key)" v-for="(vo,key) in item" :key="key">
                            <div class="num"
                                :class="[vo.today ? 'today' : '',vo.type == 'month' ? 'month' : (retract ? '' : 'disabled')]" :style="key == 0? 'color:#D93025' : key == 6? 'color:#1A73E8' : '' ">
                                {{vo.day}}
                            </div>
                            <div v-show="vo.dot && (vo.type == 'month' || retract)" class="dot"></div>
                        </div>
                    </div>
                   <!-- <div @click="open" class="retract icon"><text class="iconfont next icon-fanhui"
                            :class="[retract ? '' : 'retract_icon']" /></div> -->
                </swiper-item>
                <swiper-item>
                    <div class="date_dl" v-show="!retract || index == to_week_index" v-for="(item,index) in week_list"
                        :key="index">
                        <div class="dd" @click="item_click(vo,index,key)" v-for="(vo,key) in item" :key="key">
                            <div class="num"
                                :class="[vo.today ? 'today' : '',vo.type == 'month' ? 'month' : (retract ? '' : 'disabled')]" :style="key == 0? 'color:#D93025' : key == 6? 'color:#1A73E8' : '' ">
                ...
                <swiper-item>
                    <div class="date_dl" v-show="!retract || index == to_next_week_index"
...
<script>
    export default {
        ...
        methods: {
            ...
            item_click(item, item_index = -1,key) {
                console.log(item)
                console.log(key)
