        ...
        <view :class="{ hide: !monthOpen }" class="content">
            <view :style="{ top: positionTop + 'rpx' }" class="days">
                <view class="item" v-for="(item, index) in dates" :key="index">
                    ...
                    <view class="rest" style="color: #1A73E8 !important;" v-show="restArr.indexOf(Number(item.date)) == '-1' && item.isCurM && item.count != 0">{{item.count}}</view>
        ...
        watch:{
            "businessArr":{
                handler(newVal,oldVal){
                    ...
                    that.dates.forEach((v,i) =>{
                        arr.forEach((value,index) =>{
                            if(v.isCurM == true && v.date*1 == (index+1)){
                                v.count = value.qrocd_count
                            }
                        })
                    })
                }
