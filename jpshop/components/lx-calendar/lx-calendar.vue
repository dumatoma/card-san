<template>
    <view class="date">
        <view class="head">
            <view class="title">{{nowYear+'年 '+ nowMonth+'月'+nowDay+'日'}}</view>
            <view style="display: flex;align-items: center;justify-content: center;">
                <view class="icon" @click="switch_month_week('prev',true)"><text class="iconfont icon-fanhui"
                        style="color: #1A73E8;margin-right: 70upx;" /></view>
                <view class="icon" @click="switch_month_week('next',true)"><text class="iconfont next icon-fanhui"
                        style="color: #1A73E8;" /></view>
            </view>
        </view>
        <view style="box-sizing: border-box;padding-left: 150upx;padding-right: 35upx;">
            <view class="date_dl">
                <view class="dd" v-for="(item,index) in week" :key="index" :style=" item == '日' ? 'color:#D93025' : item == '土' ? 'color:#1A73E8' : '' ">{{item}}</view>
            </view>
            <swiper :style="{height:(retract ? 2 * 50 : (week_list.length + 1) * 50 ) + 'rpx'}" :current="current"
