<template>
    <view class="contain border_top">
        <u-navbar :placeholder='true' title="クーポン編集" :autoBack="true" :border="true" :safeAreaInsetTop="true">
        ...
        <!-- <view class="inp" v-if="info.value3">
            <view class="inpTitle">
                JANコード：
            </view>
        </view> -->
        <view class="inp" style="margin-top: 50upx;">
            ...
            <u-textarea height="200" v-model="info.remark" placeholder="" confirmType="done"></u-textarea>
        </view>
        ...
        methods: {
            ...
            getDetail(id){
              ...
              getCouponDetail(id).then((res) => {
                  if(res.code == 200){
                      ...
                      that.info.banner = data.images
                      that.info.detail = data.des
                      that.info.remark = data.remark
                      that.info.number = data.number
                      that.info.value = data.is_indate == 1? true : false
                      that.info.startDate = data.indate_start_time
                      that.info.endDate = data.indate_end_time
                      that.info.value2 = data.count == 0? false : true
                      that.info.count = data.count == 0? '利用回数を選択' : data.count
                      that.info.title = data.title
                      that.info.value3 = data.is_bar_code == 1 ? true : false
                      
                      let obj = {}
                      if(data.birthday_month != ""){
                          let temp = data.birthday_month.split(",")
                          obj.birthday_month = temp
                      }else{
                           obj.birthday_month = ""
                      }
                      if(data.sex != ""){
                          let temp = data.sex.split(",")
                          obj.sex = temp
                      }else{
                           obj.sex = ""
                      }
                      obj.is_order = data.is_order == 1? true : false
                      obj.order_time = data.order_time
                      obj.shop_count = data.shop_count
                      obj.shop_count_desc = data.shop_count_desc?data.shop_count_desc:3
                      obj.shop_time = data.shop_time
                      obj.shop_time_desc = data.shop_time_desc?data.shop_time_desc:3
                      obj.receiver = data.receiver == 1 ? 0 : 1
                      uni.setStorageSync("csvData",obj)                      
                  }else{
                      uni.showToast({
                          title:res.message,
                          icon:"none"
            ...
            nextStep() {
                ...
                if(that.info.title != ''){
                    ...
                    uni.navigateTo({
                        url: "../../coupon/setting/setting?source=edit&id="+id
                    })
                }
            }
        }
