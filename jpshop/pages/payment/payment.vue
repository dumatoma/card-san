...
<script>
    ...
    export default {
        ...
        methods: {
            ...
            toSuccess1() {
                let that = this
                if(that.using.card_type == 3){
                        that.show = true
                        that.title = 'プラン・支払い周期の変更について'
                        that.content = 'Google Playで利用料金をお支払いの場合、現在、ご利用プラン・支払い周期の変更に対応しておりません。'
                }
                if(that.using.card_type == 1){
                    if(that.type > that.using.type){
                        that.changeS("e")
                    }else{
                        that.show = true
                        that.title = 'プラン・支払い周期の変更について'
                ...
                if(that.using.card_type == 0 || that.using.card_type == 2){
                    that.changeS("e")
                }  
            },
            changeMethods(e) {
