<template>
    <view style="width: 100vw;overflow:hidden">
        ...
        <view class="content" v-if="type != 3">
                ...
                <view class="cardInner">
                    <view class="seal" v-for="(item,index) in list">
                        ...
                        <view class="sealed" v-if="item.sealed">
                            ...
                            <view class="sealTime">YY/MM/DD</view>
                        </view>
                    </view>
                </view>
            </view>
            ...
            <view class="chooseContent" @click="ale = true">
                <view class="chooseLeft">
                    スタンプ数
                </view>
                <view class="chooseRight">
        ...
        <view class="content" v-if="type == 3">
            ...
            <view class="newItem">
                ...
                <view class="newRight">
                    <view class="rightTitle" :style="use?'color:#1d1d1f' : ''">
                        設定する
                    </view>
        ...
        <view class="title" @click="ale = true">
            <view>有効期限</view>
            <u-switch v-model="value" :disabled="true" activeColor="#34C759"></u-switch>
        </view>
        <view class="newContent">
            <view class="fuTitle">オフにすると、有効期限は設定されません</view>
            <view class="newItem">
                <view class="newLeft">
                    <view class="selectContents">
            ...
            <view class="newItem">
                ...
                <view class="newRight">
                    ...
                    <view class="rightContent">
                        <view class="can">
                            <view class="canLeft">
                                <u-icon top="5" color="#aaa" name="calendar" size="34"></u-icon>
                            </view>
                            <view style="box-sizing: border-box;padding: 0 20upx;" v-text="`${endDate}`"></view>
                        </view>
                    </view>
                </view>
            </view>
        ...
        <view class="newContent" >
            ...
            <view class="newItem" v-if="value1">
        ...
        <view class="newContent">
            ...
            <view class="topImage" v-if="value2">
            ...
            <view class="caution" v-if="value2">
                画像は4枚まで掲載可
            </view>
            <view class="addImage" v-if="value2" @click="uploadBanner">
                <text>+</text>
                <view>画像を選択</view>
            </view>
           <!-- <view class="inp"  v-if="value2">
                <view class="inpTitle">
            ...
            <view class="inp"  v-if="value2">
                ...
                <u-textarea height="100" v-model="yContent" placeholder="特典内容を記入"> </u-textarea>
            </view>
        </view>
            ...
            <view class="containe">
                ...
                <view class="cardInfo">
                    ...
                    <view class="cardInfos">
                        <view class="ciItem" v-if="value == true">
                            ...
                            <view class="ciRight" v-text="`${endDate} (${week})`">
                            </view>
                        </view>
                        <view class="ciItem" v-if="value1 == true">
                            ...
                            <view class="ciRight" style="white-space: pre-wrap;">
                               {{remark}}
                            </view>
                        </view>
                    </view>
                ...
                <view class="special" v-if="value2 == true">
                 ...
                 <!--   <view class="remark">
                    ...
                    <view class="remark" style="padding-top: 0 !important;white-space: pre-wrap;">
                        {{yContent}}
                    </view>
                </view>
            </view>
        ...
        <view v-if="show1" class="mask" @touchmove.stop.prevent="">
            <view class="maskContent">
                ...
                <chooseColor :source='colorType' :Colorss="secolor" @selectColor="saveTempColor"></chooseColor>
            </view>
        </view>
        <!-- 选择数量 -->
        <u-picker @confirm="selectLogoNum" cancelText="キャンセル" confirmText="保存" :show="show2" :columns="columns">
        </u-picker>
        <!-- 选择日期 -->
        ...
        <view class="maskk" v-if="k == true"></view>
        <view class="aleMask" v-if="ale">
            <view class="maskContents">
                <view class="maskTop">
                    スタンプカード数・有効期限の変更はできません。
                </view>
                <view class="maskBottom">
                    スタンプカード数・有効期限の変更を行う場合は、新しいカードを作成する必要がございます。
                </view>
                <view class="bts" @click="ale = false">
                    OK
                </view>
            </view>
        </view>
...
<script>
    ...
    import {editCard,getCardDetail} from "@/api/index.js"
        ...
        methods: {
            getDetail(id){
                ...
                getCardDetail(id).then((res) => {
                    if(res.code == 200){
                        that.background = res.data.card.background_color
                        that.name = res.data.card.name
                        that.number = res.data.card.count
                        that.logoColor = res.data.card.seal_color
                        that.remark = res.data.card.remark
                        that.banner = res.data.card.discount_images
                        that.yContent = res.data.card.discount_content
                        // that.yTitle = res.data.card.discount_title
                        that.amount = res.data.card.amount
                        that.point = res.data.card.point
                        that.selfLogo = res.data.card.seal_icon
                        that.use = res.data.card.is_point == 0? false : true
                        that.value = res.data.card.valid_type == 0? false:true
                        that.value1 = res.data.card.is_remark == 0? false:true
                        that.value2 = res.data.card.is_discount == 0? false:true
                        let temp = []
                         for (let i = 0; i < res.data.card.count; i++) {
                             let data = {}
                             data.sealed = true
                             data.time = "YY/MM/DD"
                          ...
                          that.list = temp
                        that.svgData11 = that.changeColor(that.svgData1,res.data.card.seal_color);
                        that.svgData12 = that.changeColor(that.svgData2,res.data.card.seal_color);
                        that.svgData13 = that.changeColor(that.svgData3,res.data.card.seal_color);
                        that.svgData14 = that.changeColor(that.svgData4,res.data.card.seal_color);
                        that.svgData15 = that.changeColor(that.svgData5,res.data.card.seal_color);
                        that.svgData16 = that.changeColor(that.svgData6,res.data.card.seal_color);
                        that.svgData17 = that.changeColor(that.svgData7,res.data.card.seal_color);
                        that.svgData18 = that.changeColor(that.svgData8,res.data.card.seal_color);
                        if(res.data.card.seal_icon.includes('sv_02')){
                            that.logos[1].checked = true
                            that.currentLogo = 1
                            that.method = 1
                        }else if(res.data.card.seal_icon.includes('sv_01')){
                            that.currentLogo = 0
                             that.logos[0].checked = true
                             that.method = 1
                        }else if(res.data.card.seal_icon.includes('sv_03')){
                            that.currentLogo = 2
                             that.logos[2].checked = true
                             that.method = 1
                        }else if(res.data.card.seal_icon.includes('sv_04')){
                            that.currentLogo = 3
                             that.logos[3].checked = true
                             that.method = 1
                        }else if(res.data.card.seal_icon.includes('sv_05')){
                            that.currentLogo = 4
                             that.logos[4].checked = true
                             that.method = 1
                        }else if(res.data.card.seal_icon.includes('sv_06')){
                            that.currentLogo = 5
                             that.logos[5].checked = true
                             that.method = 1
                        }else if(res.data.card.seal_icon.includes('sv_07')){
                            that.currentLogo = 6
                             that.logos[6].checked = true
                             that.method = 1
                        }else if(res.data.card.seal_icon.includes('sv_08')){
                            that.currentLogo = 7
                             that.logos[7].checked = true
                             that.method = 1
                        }else{
                            that.fileName = res.data.card.seal_icon_name
                            that.method = 2
                        }
                        if(res.data.card.valid_type == 1){ 
                            that.days = res.data.card.valid_day
                            that.dateType = 1
                            that.showdays = (res.data.card.valid_day * 1) > 280? ((res.data.card.valid_day * 1) / 365) + '年' : (res.data.card.valid_day * 1) + "日"
                        }
                        if(res.data.card.valid_type == 2){
                            that.endDate = res.data.card.expire_time
                            that.dateType = 2
                        }
                        if(res.data.card.valid_type == 0){
                          that.dateType = 0
                        }
            ...
            toChoose(e) {
                ...
                this.show1 = true
                if(e == "background"){
                    this.secolor = this.background
                }else{
                    this.secolor = this.logoColor
                }
            },
            save() {
                uni.showLoading({
